...
@interface XFYBannerView()<UIScrollViewDelegate>
{
...
@end
@implementation XFYBannerView

+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {
    
    XFYBannerView *bannerView = [[XFYBannerView alloc] init];
    bannerView.loadBlock = loadBlock;
    return bannerView;
...
-(void)setPicModels:(NSArray<id <XFYBannerViewModelProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<XFYBannerViewModelProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
        ...
        if (self.loadBlock) {
            self.loadBlock(imageView, picM.bannerImgURL);
        }
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<XFYBannerViewModelProtocol> adM = self.picModels[tag];
...
- (void)caculateCurrentPage: (UIScrollView *)scrollView {
    ...
    if (self.picModels.count == 1) {
        _currentPage = 1;
        if ([self.delegate respondsToSelector:@selector(bannerViewDidSelectedPicModel:)]) {
            [self.delegate bannerViewDidSelectedPicModel:self.picModels[self.pageControl.currentPage]];
        }
        return;
    }
    // 确认中间区域
